//
//  BDChangeBirthdayViewController.m
//  SuperDesigner
//
...
@interface BDChangeBirthdayViewController ()
@property(nonatomic, weak) UIView *backView;
@property(nonatomic, strong) UITextField *textTF;
@end

@implementation BDChangeBirthdayViewController
...
-(void) setUp {
    ...
    textName.placeholder = @"请输入您的生日(格式:1992-6-6)";
...
-(void) clickBtn {
    ...
    NSLog(@"这个用户的ID---%@", IDNum);
    NSLog(@"这个用户输入的---%@",  self.textTF);
    NSDictionary *dictParams = @{
                                 @"id": IDNum,
                                 @"birthday" : self.textTF.text
                                 
                                 };
    
    [[BDBaseHttpTool sharedInstance] getWithUrl:url_changeUserBirthday parameters:dictParams sucess:^(id json) {
