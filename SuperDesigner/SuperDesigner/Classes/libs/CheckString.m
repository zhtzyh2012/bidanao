...
//    NSString *emailRegex = @"1[3|4|5|7|8][0-9]\\d{4,8}";
...
//    NSString *emailRegex = @"[A-Z0-9a-z._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,4}";
    NSString * pattern = @"^([0-9A-Za-z\\-_\\.]+)@([0-9a-z]+\\.[a-z]{2,3}(\\.[a-z]{2})?)$";
    NSPredicate *pred = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", pattern];
...
#pragma 判断用户信息是否正确(验证是否是手机号或邮箱)
+ (BOOL)isCountRight:(NSString *)candidate
{
    NSRange range = [candidate rangeOfString:@"@"];//查找子串，找不到返回NSNotFound 找到返回location和length
    ...
    if (![phoneTest evaluateWithObject:candidate]) {
        return NO;
    }
